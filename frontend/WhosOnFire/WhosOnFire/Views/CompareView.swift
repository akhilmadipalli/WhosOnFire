...
struct CompareView: View {
    ...
    func updatePlayerStats(playerA: Player, playerB: Player) {
        ...
        print(
            "Standard devs calculated for \(playerA.position)_\(selectedSeason)"
        )
        ...
        ScrollView {
            VStack(spacing: 24) {
                ...
                if playerA != nil && playerB != nil {
                    HStack {
                        Text(playerA!.player_display_name)
                            .fontWeight(.bold)
                            .foregroundStyle(.primary)
                            .frame(maxWidth: .infinity, alignment: .leading)
                        Spacer().frame(width:85)
                        Text(playerB!.player_display_name)
                                .fontWeight(.bold)
            ...
            ToolbarItem(placement: .topBarTrailing) {
                Menu {
                    Picker("Season", selection: $selectedSeason) {
                        Text("All Seasons").tag("All")
                        ForEach(Array(Set(stats.map { $0.season })).sorted(), id: \.self) { year in
                            Text(String(year)).tag(String(year))
                        }
                    }
                } label: {
                    let iconName = selectedSeason == "All" ? "calendar" : "calendar.fill"
                    
                    Label("Season", systemImage: iconName)
